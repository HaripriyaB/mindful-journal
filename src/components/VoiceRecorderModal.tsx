...
export const VoiceRecorderModal: React.FC<VoiceRecorderModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/70 backdrop-blur-sm overflow-y-auto">
      <div className="bg-[#242731] border border-[#373b47] text-slate-100 rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="px-6 py-4 border-b border-[#373b47] flex items-center justify-between bg-[#1c1e26] shrink-0">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Speak in Hindi, Tamil, Telugu, Marathi, or any language &middot; Translated into elegant prose
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-5 overflow-y-auto flex-1">
                ...
                <button
                  ...
                  className={`p-2.5 rounded-xl text-left border transition text-xs flex flex-col justify-between ${
                    selectedLanguage === lang.code
                      ...
                      : 'bg-[#1c1e26] border-[#373b47] text-slate-300 hover:bg-[#2d313d]'
                  }`}
                >
          ...
          {/* Recording Area */}
          <div className="bg-[#1c1e26] border border-[#373b47] rounded-2xl p-5 flex flex-col items-center justify-center text-center space-y-4">
              ...
              <button
                ...
                className={`relative z-10 w-20 h-20 rounded-full flex items-center justify-center shadow-lg transition-all ${
                  isListening
                    ...
                    : 'bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-slate-950 font-bold'
                }`}
              >
                {isListening ? (
            ...
            {audioBlobUrl && !isListening && (
              <div className="flex items-center space-x-3 bg-[#242731] border border-[#373b47] px-4 py-2 rounded-xl text-xs">
                <button
                  type="button"
                ...
                <span className="text-slate-300 font-medium">Recorded Voice Memo ({audioDuration}s)</span>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-[#1c1e26] border border-[#373b47] focus:border-amber-500 rounded-xl p-3.5 text-xs text-slate-100 focus:outline-none transition resize-none placeholder:text-slate-500 font-sans"
            />
          </div>
          ...
          {liveTranscript && !translationResult && (
            <button
              ...
              className="w-full flex items-center justify-center space-x-2 bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-slate-950 font-bold py-3 px-4 rounded-xl shadow-md transition disabled:opacity-50 text-xs"
            >
              {isTranslating ? (
                <>
          ...
          {translationResult && (
            <div className="bg-[#1c1e26] border border-amber-500/40 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-slate-400">
                  Mood: <strong className="text-slate-200 capitalize">{translationResult.suggestedMood}</strong>
                </span>
              </div>

              <div>
                <h4 className="font-serif font-semibold text-slate-100 text-sm mb-1">
              ...
              {translationResult.originalText && (
                <div className="pt-2 border-t border-[#373b47] text-[11px] text-slate-400 italic">
        ...
        {/* Footer Actions */}
        <div className="px-6 py-4 bg-[#1c1e26] border-t border-[#373b47] flex items-center justify-end space-x-3 shrink-0">
          <button
            type="button"
            onClick={onClose}
